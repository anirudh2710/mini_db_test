...
private:
    ...
    /*!
     * Returns the beginning of the slot array, which grows backwards and its
     * valid range is `[-1..-GetMaxSlotId()]`.
     * storage/VarlenDataPage.cpp for details).
     */
    constexpr SlotData*
    GetSlotArray() const {
        return (SlotData*)(m_pagebuf + PAGE_SIZE);
    }

    /*!
     * Copies a record into an unoccupied space at the beginning of the free
     * space and stores the offset and length into the slot \p sid. The caller
     * should guarantee that the free space is large enough for hodling the
     * record. This never fails, and upon return, `rec.GetRecordId().sid` is
     * set to `sid`.
     */
